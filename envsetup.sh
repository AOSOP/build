...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^optimized_") ; then
        OPTIMIZED_BUILD=$(echo -n $1 | sed -e 's/^optimized_//g')
        export BUILD_NUMBER=$( (date +%s%N ; echo $OPTIMIZED_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        OPTIMIZED_BUILD=
    fi
    export OPTIMIZED_BUILD
...
function breakfast()
{
    target=$1
    local variant=$2
    OPTIMIZED_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/aosop/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the AOSOP model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch optimized_$target-$variant
        fi
    fi
    return $?
}
